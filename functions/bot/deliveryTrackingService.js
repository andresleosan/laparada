...
exports.notificarEstadoEntrega = notificarEstadoEntrega;
exports.actualizarProgresoEntrega = actualizarProgresoEntrega;
exports.obtenerEstadisticasEntregas = obtenerEstadisticasEntregas;
exports.generarReporteEntregasDelDia = generarReporteEntregasDelDia;
const admin = __importStar(require("firebase-admin"));
const db = admin.firestore();
/**
 * Envía actualizaciones de estado de entrega al cliente por WhatsApp
 */
async function notificarEstadoEntrega(domicilioId, nuevoEstado) {
    try {
        // Obtener domicilio
        ...
        const domicilio = domicilioDoc.data();
        if (!domicilio) {
            ...
            case 'en_camino':
                ...
                if (domicilio && domicilio.domiciliarioId) {
                    ...
                    if (domiciliarioDoc.exists) {
                        const domiciliario = domiciliarioDoc.data();
                        if (domiciliario) {
 ...
 */
async function registrarNotificacionEntrega(domicilioId, estado, mensaje) {
    try {
 ...
 */
async function actualizarProgresoEntrega() {
    try {
        ...
        for (const doc of domiciliosEnCamino.docs) {
            ...
            const salida = domicilio.horaEnCamino instanceof admin.firestore.Timestamp
 ...
 */
async function obtenerEstadisticasEntregas(fecha) {
    try {
            ...
            switch (domicilio.estado) {
                ...
                case 'entregado':
                    ...
                    if (domicilio.creadoEn && domicilio.entregadoEn) {
                        const inicio = domicilio.creadoEn instanceof admin.firestore.Timestamp
                            ? domicilio.creadoEn.toDate()
                            : new Date(domicilio.creadoEn);
                        const fin = domicilio.entregadoEn instanceof admin.firestore.Timestamp
        ...
        if (tiemposEntrega.length > 0) {
            totales.tiempoPromedioEntrega =
                Math.round((tiemposEntrega.reduce((a, b) => a + b, 0) / tiemposEntrega.length) * 100) / 100;
        }
        return totales;
    }
    catch (error) {
 ...
 */
async function generarReporteEntregasDelDia() {
    try {
