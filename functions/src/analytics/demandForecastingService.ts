...
export class DemandForecastingService {
   ...
   */
  async obtenerDatosHistoricos(dias: number = 90): Promise<DemandData> {
    const ahora = new Date();
    const hace90Dias = new Date(ahora.getTime() - dias * 24 * 60 * 60 * 1000);

    const ventasSnapshot = await this.db
      .collection('ventas')
      .where('createdAt', '>=', Timestamp.fromDate(hace90Dias))
      .get();
    ...
    const desviacionEstándar =
      Math.sqrt(
        valoresHora.reduce((sum, val) => sum + Math.pow(val - promedioHora, 2), 0) /
          valoresHora.length
      ) || 0;
   ...
   */
  async pronosticarProximaHora(): Promise<DemandForecast> {
    ...
    const demandaHoyHasta = Array.from(datosHistoricos.ventasPorDia.values()).filter(
      (_, i) => new Date().getDate() - i === new Date().getDate()
    );
    ...
    const demandaPronosticada = Math.round(
      datosHistoricos.promedioHora * factorHistorial * 0.5 +
   ...
   */
  async pronosticarSemana(): Promise<DemandForecast[]> {
    const pronosticos: DemandForecast[] = [];
    const datosHistoricos = await this.obtenerDatosHistoricos();
   ...
   */
  async guardarPronostico(pronostico: DemandForecast): Promise<string> {
   ...
   */
  async calcularPrecision(dias: number = 30): Promise<number> {
   ...
   */
  async generarRecomendacionProduccion(): Promise<{
   ...
   */
  private calcularFactorEstacionalidad(hora: number): number {
   ...
   */
  private esHoraPico(hora: number): boolean {
   ...
   */
  async obtenerTendencia(): Promise<{
    dia: string;
    demanda: number;
    ...
    return ventasPorDia.map((entry, index) => {
      ...
      if (index > 0) {
        ...
        if (actual > anterior * 1.1) tendencia = 'mejorando';
        else if (actual < anterior * 0.9) tendencia = 'empeorando';
      }
